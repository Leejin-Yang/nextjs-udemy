...
지금까지 설명에서 session이란 jwt이다. jwt를 nextauth가 자동으로 관리하며 nextauth가 브라우저에 저장하는 대상으로서 브라우저에는 이 토큰을 가진 쿠키가 생성된다. nextauth는 쿠키와 그 쿠키에 저장된 토큰을 통해 사용자의 로그인 여부를 확인한다.
...
- PATH method
- 인증된 사용자가 보낸 요청인지 확인
- 토큰에 부호화해서 사용자 이메일을 가져올 수 있다.
- body로 받은 기존과 새 비밀번호를 가져온다.
- db에 연결해 해당 이메일의 유저를 찾는다
  - 유저가 없다면 404
- 입력 받은 비밀번호와 db에 저장된 비밀번호 비교
  - 불일치하면 403
